...
export function calculateDevelopmentDelta({player,coach,minutes,games,role,injury}){
 ...
 const dev={Explosivo:1.2,Rápido:.8,Normal:.25,Lento:-.35}[player.dna?.development]||0;
 const opportunity=clamp((minutes-18)/8,-1.2,1.7)*clamp(games/65,.35,1.1);
 const roleBonus={Superestrella:.15,Titular:.35,"Sexto hombre":.25,"Rotación":0,Proyecto:-.25}[role]||0;
 const ageCurve=age<=21?1.6:age<=24?1.15:age<peak?.55:age<=peak+2?.05:age<=33?-.75:age<=36?-1.45:-2.1;
 const ceiling=gap<=1?-1.1:gap<=4?-.35:gap>=12?.45:0;
 const health=injury?.severe?-1.5:injury?-.35:0;
 const coachEffect=(coach?.development||4)/12;
