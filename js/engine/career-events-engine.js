...
function objective(type,target){
 ...
 return {id:`${type}_${Date.now()}_${Random.next()}`,type,title:data[0],description:data[1],stat:data[2],unit:data[3],target,status:'active',reward:{reputation:3,popularity:2,morale:3},penalty:{morale:-2}};
}
export function ensureSeasonObjectives(game){
 ...
 const pool=[primary,'wins','games','per'].filter((v,i,a)=>a.indexOf(v)===i),difficulty=migrateDifficulty(game);
 c.objectives=pool.slice(0,3).map(type=>{const base=targetFor(p,type);return {...objective(type,Math.max(1,Math.round(base*difficulty.objective))),season:game.season,difficulty:difficulty.id};});
 return c.objectives;
}
...
export function generateDynamicEvent(game){
 ...
 const difficulty=migrateDifficulty(game);
 if(seasons<1||roll(1,100)>Math.min(82,58*difficulty.eventChance))return null;
