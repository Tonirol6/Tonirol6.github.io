...
export function stabilizeGame(game){
 ...
 game.system??={};game.system.beta='1.1-phase2';game.system.stabilityPasses=(game.system.stabilityPasses||0)+1;
 ...
 game.seasonResults=cap(game.seasonResults,60);
...
export function diagnoseGame(game){
 ...
 const results=game?.seasonResults||[];for(const r of results){const e=game?.encyclopedia?.seasons?.find(x=>x.season===r.season);const u=game?.universe?.seasonHistory?.find(x=>x.season===r.season);if(e&&e.nba?.champion!==r.nba?.champion)issues.push(`Campeón incoherente en ${r.season}`);if(u&&u.nbaChampion!==r.nba?.champion)issues.push(`Cronología incoherente en ${r.season}`);}
